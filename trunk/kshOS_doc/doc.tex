...
Hlavní myšlenka tohoto software byla pøevzata z GNU/Linux. Z dùvodu absence zavadìèe jádra a dalších procedur, nutných ke startu "bìžného" operaèního systému, jsme si však mohli dopøát nìkolik zjednodušení.\\
Pro zavedení jádra operaèního systému jsme využili jednoduše metodu \textit{void main()}. Takže ihned po spuštìní programu se nám zavede jádro. Jádro spustí (vytvoøí instance) potøebné manažery. V našem pøípadì jsou to, manažer uživatelù (\textit{UserManager.java}), manažer uživatelských rozhraní (\textit{UIManager.java}) a manažer nastavení (\textit{PropertyManager.java}).\\
Po zavedení managerù je spuštìn \textit{INIT} proces. PID (Process IDentifier) je vždy rovno 1. Zde je vidìt analogie s GNU/Linux. V operaèním systému Linux se tento proces (\textit{INIT}), nastavený jako daemon process, používá k startování jiných procesù jádra. \textit{INIT} proces je koøenem stromu procesù, který za bìhu OS (Operating System) vytváøí. V naší implementaci je \textit{INIT} pouze prázdný process, který nic nedìlá. Ovšem jeho implementace umožòuje použití ke stejnému úèelu jako v Linuxu.\\
Po zavedení a spuštìní \textit{INIT} procesu je spuštìno pøihlašovací okno. Zde je umožnìno pøihlášení uživatelù. Naše implementace neobsahuje ovìøení heslem. Po zadání uživatelského jména získá uživatel terminál, pomocí kterého lze komunikovat se systémem.
...
Ovládání správce je intuitivní. Po spuštìní (viz pøedchozí èást) se objeví okno pøihlášení. Po zadání uživatelského jména (systémem akceptovaná uživatelská jména: hauz, k4chn1k, sysek, guest) a potvrzení dojde k pøihlášení uživatele k virtuálnímu stroji. Zde je možné používat základní pøíkazy shellu, vèetnì jejich argumentù a pøesmìrování vstupù resp. výstupù.
Implementované pøíkazy: \textit{cat, cd, echo, exit, kill, kshell, ls, man, ps, pwd, shutdown, sort}.
Obecná syntaxe:
\begin{verbatim}
cmd1 [file]* [< ifile] [| cmd\_next]* [> ofile]
\end{verbatim}
Výjimkou oproti zadaní je pøíkaz \textit{ps}. Ten lze použít s pøepínaèem \textit{-u}, èímž se zobrazí pouze uživatelovy procesy. Dále byl pøidán pøíkaz \textit{clr}, který smaže obsah konzole.
...
Práce byla velkým pøínosem. Studium by mìlo obsahovat více takovýchto týmových projektù. Dále nám práce umožnila nahlédnout, alespoò trošku,
pod kapotu souèasných operaèních systémù. Tyto zkušenosti jsou k nezaplacení.\\[5mm]
